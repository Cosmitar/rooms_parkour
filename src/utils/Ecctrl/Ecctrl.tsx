...
import { useEffect, useRef, useMemo, type ReactNode, forwardRef, type RefObject, MutableRefObject } from 'react'
...
const Ecctrl = forwardRef<RapierRigidBody, EcctrlProps>(
  (
    ...
    const actionDieAnimation = !animated ? null : useGame(state => state.die)
    ...
    const slopeRayOriginRef = useRef<THREE.Mesh>() as MutableRefObject<THREE.Mesh>
    ...
    const moveCharacter = (_: number, run: boolean, slopeAngle: number, movingObjectVelocity: THREE.Vector3) => {
       ...
       */
      const characterRotated = Math.sin(characterModelIndicator.rotation.y).toFixed(3) == Math.sin(modelEuler.y).toFixed(3)
    ...
    if (isInsideKeyboardControls) {
      useEffect(() => {
        ...
        // Trigger key subscribe for special animation
        const unSubscribeActionDie = subscribeKeys(
          state => state.die,
          value => {
            if (value) {
              animated && actionDieAnimation()
            }
          }
        )
